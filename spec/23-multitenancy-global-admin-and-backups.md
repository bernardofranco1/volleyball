...
- **2026-07-30 incident (post-launch)**: authenticated pages froze (300 s timeouts). Root cause: postgres.js **pipelines** queries onto busy connections once in-flight concurrency exceeds pool `max`, and Supavisor transaction-mode pooling does not support pipelining — responses desync and the connection wedges permanently. The spec/23 console queries (parallel `listAllTenants` + concurrent layout/page/metadata queries) crossed that threshold for the first time. Fix: `max_pipeline: 0` in `src/db/index.ts` (excess queries wait in-process for a free connection) + the tenant-layout switcher now uses a single-query `listTenantsForSwitcher()`. **Never remove `max_pipeline: 0` while on the transaction pooler.**
- **Platform People console** (`/admin/access`, post-launch addendum): global admins manage ALL access in one place — provision accounts (one-time temp password), grant/revoke the global-admin flag (self-revocation blocked), grant/revoke per-tenant roles (last-admin guard), and reset passwords. Provisioning logic shared with the tenant Access page via `src/lib/user-provisioning.ts`. Tenant admins keep their own tenant-scoped Access page.
