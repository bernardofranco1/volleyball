// Full demo seed (spec/13). Four competitions — one per discipline — each with a
// finished match and a live (mid-match) match, under the existing demo tenant so
// the seeded admin sees them on login. Matches are produced through the real
// engine (appendMatchEvent), so events + denormalised columns are correct by
// construction and the engine is exercised end-to-end.
//
// Idempotent: each competition is wiped (FK-safe) and rebuilt on every run.
...
function setStartPayloads(
  ...
  // Lineups are only needed when we play real rallies (the live partial set);
  // synthetic SET_END sets don't require an active lineup.
...
/**
 * Build the full payload sequence for a match. Completed sets use synthetic
 * SET_END (the results-import path — scores with no rally events), then MATCH_END
 * on the clinching set. A trailing `partial` set is played with real rallies so a
 * live match shows true mid-set state.
 */
...
const SEED_TS = "2026-07-01T10:00:00.000Z";

/**
 * Replay a payload list through the discipline engine in-memory, then bulk-insert
 * the events (with per-event denormalised columns) and update the matches row.
 * Avoids per-event DB round-trips, the server state cache, and realtime
 * broadcasts entirely — deterministic and fast.
 */
async function generateMatch(
  ...
  for (const payload of pls) {
    ...
    // A rally can auto-emit a technical time-out (beach) that pauses play;
    // resolve it so the following rallies are accepted.
  ...
  // Per-event denormalised columns via a fresh re-reduce.
...
/** Build + persist a match (config resolved from its competition's overrides). */
...
async function seedLight() {
  ...
  // Live match: play to 8-6, then illustrate a Light-specific fault by team B.
