...
export function LightActionBar() {
  ...
  if (state.rallyPhase === "TIMEOUT_ACTIVE")
    return (
      <Banner>
        <Primary onClick={() => dispatch({ type: "TIMEOUT_END", team: state.activeTimeoutTeam ?? "A" })}>
  ...
  if (state.rallyPhase === "LINEUP_PENDING")
    return (
      <Banner>
        <span className="text-sm text-score-dim">Confirm both lineups above to begin.</span>
      </Banner>
    );
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        {(["A", "B"] as const).map((t) => (
          <div key={t} className="flex flex-wrap items-center justify-center gap-1.5">
            ...
            <Secondary onClick={() => dispatch({ type: "JUMP_SERVE_FOOT_FAULT", team: t })}>
              Serve fault
            </Secondary>
            <Secondary onClick={() => dispatch({ type: "ATTACK_ARC_FAULT", team: t })}>
              Arc fault
            </Secondary>
          </div>
        ))}
      ...
      <p className="text-center text-[11px] text-score-dim">
        Faults award the point to the opponent.
      </p>
...
function SubPanel({ team, onClose }: { team: TeamId; onClose: () => void }) {
  const { state, dispatch, rosterA, rosterB } = useLightMatch();
  const set = activeSet(state)!;
