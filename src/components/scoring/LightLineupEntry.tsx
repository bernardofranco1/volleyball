...
import { type PlayerLite, useLightMatch } from "@/lib/light-match-context";

// Light confirms both teams in one event (no team tablets). Scorer sets each
// team's rotation order (position 1 serves first), then confirms.
export function LightLineupEntry() {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface-raised p-4">
      ...
      <p className="mb-4 text-center text-xs text-score-dim">Position 1 serves first.</p>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <TeamColumn teamName={teamAName} roster={rosterA} lineup={lineupA} setLineup={setLineupA} n={n} />
        <TeamColumn teamName={teamBName} roster={rosterB} lineup={lineupB} setLineup={setLineupB} n={n} />
      </div>
      <button
        type="button"
        onClick={confirm}
        ...
        {lineup.map((pid, idx) => (
          <label key={idx} className="flex items-center gap-2 text-sm">
            ...
            <select
              value={pid}
              onChange={(e) =>
                setLineup((prev) => prev.map((v, i) => (i === idx ? e.target.value : v)))
              }
